...
# ============================================================================
# RAG SYSTEM INTERFACE (for rag_client.py)
# ============================================================================

class RAGRequest(BaseModel):
    """
    Request format for RAG CLI (rag.py).
    
    Matches the expected input structure.
    """
    record_id: str
    ontologies: List[Dict[str, Any]]  # Each has: {ontology, query_texts}
    ...
    class Config:
        ...
        #   "record_id": "SAMN123",
        #   "ontologies": [
        #     {"ontology": "UBERON", "query_texts": ["wound", "infection"]},
        #     {"ontology": "MONDO", "query_texts": ["wound infection"]}
        #   ]
        # }
        schema_extra = {
            "example": {
...
class RAGResponse(BaseModel):
    """
    Response format from RAG CLI.
    
    This gets converted to RAGOutput by rag_client.py.
    """
    record_id: str
    buckets: List[RAGBucket]
