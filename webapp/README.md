...
- **Columns:** Select which discovered fields appear in the table, or click **Select all** to check every field at once. Use **Search columns** to filter the list by column name as you type (case-insensitive); clearing the search restores the full list. Searching preserves your selections, and **Select all** still checks every field, including fields hidden by the search. You can uncheck individual fields before clicking **Apply columns**. Nested fields display with a `›` separator.
